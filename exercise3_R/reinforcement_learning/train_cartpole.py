...
def run_episode(env, agent, deterministic,epsilon, do_training=True, rendering=False, max_timesteps=1000):
    """
    ...
    while True:
        ...
        action_id = agent.act(state=state, deterministic=deterministic,epsilon = epsilon)
...
def train_online(env, agent, num_episodes, model_dir="./models_cartpole", tensorboard_dir="./tensorboard"):
    ...
    for i in range(num_episodes):
        ...
        stats = run_episode(env, agent, deterministic=False, do_training=True,epsilon = epsilon)
        ...
        if i % eval_cycle == 0:
            ...
            if mean(eval_cycle_reward) > 1000:
